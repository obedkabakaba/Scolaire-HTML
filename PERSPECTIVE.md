...
Apparence optionnelle dans **Mon profil → Apparence → Perspective**, inspirée de la maquette architecturale crème, vert profond et terre cuite. Navigation horizontale sur ordinateur, agenda et aide à droite, typographie sérif et 30 illustrations SVG par rubrique. Les 33 rubriques, dont la coque Super Admin, sont couvertes. Les documents imprimables et pages publiques conservent leur présentation.
...
Le serveur doit autoriser `perspective` dans `THEMES_AUTORISES`. Le service worker v71 précache ses ressources en plus d’Élan.

## Vérification

`node test-perspective-browser.cjs` : vrais scripts partagés avec API simulées, tableau de bord, alternance Perspective/Élan/Yohali, conservation des données, profil et rechargement, quatre positions de navigation, tiroir mobile, 32 écrans à 1536 et 390 px, coque Super Admin, migration du cache v70 vers v71 et ressources hors connexion. Les vues inventoriées utilisent leur coque sans exécuter les opérations métier. `PERSPECTIVE_BROWSER` sélectionne un Chromium installé ; `PERSPECTIVE_SHOT` indique le chemin de capture.
