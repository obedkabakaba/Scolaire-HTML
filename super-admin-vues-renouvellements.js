...
(function enregistrerQuandPret() {
  ...
  function modalAction(ligne, mode) {
    ...
    var contenu = validation
      ...
      : '<p class="sa-texte">Le motif sera envoyé à l’école pour qu’elle puisse corriger la référence ou contacter le support.</p>'
    ...
    m.querySelector('[data-valider]').onclick = async function(){
      ...
      try {
        ...
        m.fermer(); SA.toast(r.message || 'Demande mise à jour.','succes'); SA.rafraichirVue();
      } catch(e) { erreur.hidden=false; erreur.textContent=e.message||'Opération impossible.'; }
  ...
  SA.enregistrerVue('renouvellements', {
    ...
    async rendu(conteneur, params) {
      ...
      var r = await SA.api(SA.url('/super-admin/renouvellements',{statut:statut==='tous'?undefined:statut}));
      ...
      conteneur.innerHTML = '<section class="sa-section"><div class="sa-grille-stats">'
        ...
        + '</select><span class="sa-muet">Actualisation automatique toutes les 8 s</span></div>'
        + ui.tableau({
          colonnes:[
            ...
            {cle:'mode',titre:'Mode',rendu:function(l){return l.mode_paiement==='agent'?'Agent':'Dépôt'+(l.reseau_depot?' · '+esc(l.reseau_depot):'')}},
          ...
          ],lignes:lignes,vide:'Aucune demande de renouvellement'
        }) + '</section>';
      var par={};lignes.forEach(function(l){par[l.id]=l});
      conteneur.querySelectorAll('[data-renouv]').forEach(function(b){b.onclick=function(){var l=par[b.dataset.id];if(l)modalAction(l,b.dataset.renouv)}});
      var f=document.getElementById('renouv-filtre');if(f)f.onchange=function(){SA.majParams({statut:f.value});SA.rafraichirVue()};
      clearTimeout(window.__ardoiseRenouvellementsTimer);
      window.__ardoiseRenouvellementsTimer=setTimeout(function(){if(location.hash.indexOf('#/renouvellements')===0)SA.rafraichirVue()},8000);
    }
  });
