...
(async()=>{
 ...
 const browser=await chromium.launch(process.env.VIBRA_BROWSER?{executablePath:process.env.VIBRA_BROWSER,headless:true}:{headless:true});
 ...
 await ctx.addInitScript(u=>{
   ...
   localStorage.setItem('ardoise_access_token','test');localStorage.setItem('ardoise_theme','vibra');
 ...
 await page.goto(base+'/dashboard-directeur.html');await page.waitForSelector('.vb-action');await page.waitForTimeout(650);
 assert.equal(await page.locator('.vb-action').count(),4);assert.equal(await page.locator('.vb-dock a').count(),3);
 assert.equal(await page.locator('.vb-action[href="notes.html"]').count(),1);
 assert.ok((await page.locator('.vb-main #liste-evenements').textContent()).includes('Réunion pédagogique'));
 const campus=await page.locator('.vb-campus img').evaluate(n=>({loaded:n.complete&&n.naturalWidth>0,src:n.getAttribute('src')}));assert.ok(campus.loaded);assert.equal(campus.src,'public/vibra/campus.webp');
 assert.equal(await page.locator('.vb-main #stat-eleves').textContent(),'428');
 ...
 for(const theme of ['fluide','vibra','orbite','vibra','nexus','elan','perspective','recre','yohali','orbite','vibra']){
   ...
   assert.equal(await page.locator('.vb-shell').isVisible(),theme==='vibra');
   assert.equal(await page.evaluate(t=>document.querySelector(t==='vibra'?'.vb-main > .grille-stats':t==='fluide'?'.fl-main > .grille-stats':t==='orbite'?'.ob-main > .grille-stats':'.contenu > .grille-stats')===window.obStatNode,theme),true);
 }
 await page.locator('#stat-eleves').click();assert.ok(await page.evaluate(()=>obClicks)>0);
 await page.evaluate(()=>document.querySelector('#stat-eleves').textContent='429');assert.equal(await page.locator('.vb-main #stat-eleves').textContent(),'429');
 for(const compact of ['oui','non'])for(const position of ['gauche','droite','haut','bas']){
   ...
   const box=await page.locator('.barre-laterale').boundingBox();assert.equal(Math.round(box.x),18);assert.equal(Math.round(box.width),104);
   ...
   assert.ok(inner.itemWidth>=80&&inner.itemRight<=inner.railRight&&inner.brandBottom<=inner.itemTop,JSON.stringify({position,inner}));
 }
 await page.evaluate(()=>ArdoiseDisposition.definir('gauche',true));await page.waitForTimeout(650);
 await page.keyboard.press('Control+k');await page.locator('#vb-tools input').fill('comptabilite');
 assert.equal(await page.locator('#vb-tools nav a').count(),1);assert.equal(await page.locator('#vb-tools nav a').getAttribute('href'),'comptabilite.html');
 await page.keyboard.press('Escape');assert.equal(await page.locator('#vb-tools').isVisible(),false);
 await page.evaluate(()=>{const li=document.createElement('li');li.textContent='Conseil de classe';document.querySelector('#liste-evenements').prepend(li);});await page.waitForFunction(()=>document.querySelector('.vb-events').textContent.includes('Conseil de classe'));
 const mainBox=await page.locator('.vb-main').boundingBox(),asideBox=await page.locator('.vb-aside').boundingBox();assert.ok(mainBox.x+mainBox.width<=asideBox.x);
 await page.emulateMedia({media:'print'});assert.equal(await page.locator('#stat-eleves').isVisible(),true);assert.equal(await page.locator('#liste-evenements').isVisible(),true);assert.equal(await page.locator('.vb-top').isVisible(),false);await page.emulateMedia({media:'screen'});await page.waitForTimeout(250);console.log('After print',await page.evaluate(()=>({media:matchMedia('print').matches,padding:getComputedStyle(document.querySelector('.contenu')).padding,theme:document.documentElement.dataset.theme,bg:getComputedStyle(document.body).backgroundColor})));await page.reload();await page.waitForSelector('.vb-campus img');await page.waitForTimeout(300);
 await page.evaluate(()=>window.scrollTo(0,0));
 await page.screenshot({path:process.env.VIBRA_SHOT||path.join(root,'vibra-preview.png')});
 await page.setViewportSize({width:390,height:844});await page.evaluate(()=>window.scrollTo(0,0));await page.waitForTimeout(150);
 await page.screenshot({path:(process.env.VIBRA_SHOT||path.join(root,'vibra-preview.png')).replace('.png','-mobile.png')});
 await page.goto(base+'/mon-profil.html');await page.waitForSelector('.carte-theme[data-theme="vibra"]');
 await page.setViewportSize({width:1280,height:590});
 await page.evaluate(()=>ArdoiseDisposition.definir('haut',true));
 await page.locator('.carte-theme[data-theme="vibra"]').scrollIntoViewIfNeeded();
 const profileLink=await page.locator('.barre-laterale .nav-item').first().boundingBox();assert.ok(profileLink.width>=80);
 if(process.env.VIBRA_SHOT)await page.screenshot({path:process.env.VIBRA_SHOT.replace('.png','-profil.png')});
 ...
 await page.locator('.carte-theme[data-theme="nuit"]').click();assert.equal(await page.locator('.vb-banner').isVisible(),false);
 await page.waitForFunction(()=>getComputedStyle(document.getElementById('choix-position').closest('.champ-disposition')).display!=='none');
 await page.locator('.carte-theme[data-theme="vibra"]').click();assert.equal(await page.locator('.vb-banner').isVisible(),true);
 await page.reload();await page.waitForSelector('.vb-banner');assert.equal(await page.locator('.carte-theme[data-theme="vibra"]').getAttribute('aria-pressed'),'true');
 ...
 for(const name of scenes.filter(p=>p!=='super-admin')){
   ...
   for(const width of [1536,1024,390]){
     ...
     const state=await page.evaluate(()=>({workspace:!!document.querySelector('.vb-banner'),overflow:document.documentElement.scrollWidth-innerWidth}));
     ...
     if(process.env.VIBRA_SHOT&&width===1536&&['classes','inscriptions'].includes(name))await page.screenshot({path:process.env.VIBRA_SHOT.replace('.png','-'+name+'.png')});
   }
 }
 ...
 await page.waitForSelector('.sa-principal .vb-banner');
 ...
 await prof.addInitScript(()=>{localStorage.setItem('ardoise_user',JSON.stringify({id:7,ecole_id:1,roles:['professeur']}));localStorage.setItem('ardoise_token','test');localStorage.setItem('ardoise_access_token','test');localStorage.setItem('ardoise_theme','vibra');});
 ...
 const pp=await prof.newPage();await pp.goto(base+'/espace-professeur.html?shell');await pp.evaluate(()=>document.querySelector('.mise-en-page').style.display='grid');await pp.waitForSelector('.vb-banner');
 assert.equal(await pp.locator('.vb-action[href="frais-scolaires.html"]').count(),0);
 // Les professeurs participent aux concours d'admission : cet accès est prévu par session.js.
 assert.equal(await pp.locator('.vb-action[href="inscriptions.html"]').count(),1);
 await pp.locator('.vb-dock button').last().click();
 assert.equal(await pp.locator('#vb-tools a[href="abonnements.html"],#vb-tools a[href="comptabilite.html"]').count(),0);
 ...
 await op.evaluate(async()=>{await caches.open('ardoise-v78-coquille');await navigator.serviceWorker.register('/sw.js');await navigator.serviceWorker.ready;});
 await op.waitForFunction(()=>navigator.serviceWorker.controller!==null);
 const keys=await op.evaluate(()=>caches.keys());assert.ok(keys.includes(cacheVersion+'-coquille'));assert.ok(!keys.includes('ardoise-v78-coquille'));
 await offline.setOffline(true);
 const ok=await op.evaluate(async()=>Promise.all(['theme-vibra.css','theme-vibra.js','public/vibra/campus.webp','public/vibra/symbole.svg','public/vibra/people.svg','public/vibra/pen.svg','public/vibra/card.svg','public/vibra/send.svg'].map(async p=>{const r=await fetch(p);return r.ok&&(await r.text()).length>0;})));assert.ok(ok.every(Boolean));await offline.close();
 await browser.close();server.closeAllConnections();server.close();
 assert.deepEqual(failures,[]);console.log('Vibra: campus, actions, original agenda, stat node restoration, search, themes, profile, mobile, role filtering and offline passed.');
