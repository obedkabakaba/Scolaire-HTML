/* Élan : décoration progressive, sans requête métier ni réécriture des écrans. */
(function () {
  'use strict';
  var html = document.documentElement;
  var base = new URL('public/elan/', document.currentScript.src);
  var page = location.pathname.split('/').pop().replace(/\.html$/, '');
  var scenes = {
    'dashboard-directeur':['accueil','Une belle journée commence ici.','Votre école, vos projets, votre rythme.'],
  ...
  function dessinerEntete(contenu) {
    if (contenu.querySelector(':scope > .elan-hero')) return;
    var scene = scenePour(page), home = /^(dashboard-directeur|espace-)/.test(page);
    var hero = node('section','elan-only elan-hero' + (home ? ' elan-home-hero' : ''));
    hero.setAttribute('aria-label','Votre espace Élan');
    var copy = node('div','elan-hero-copy');
    copy.append(node('p','elan-kicker','Ardoise · Élan'),node('h2','',scene[1]),node('p','',scene[2]));
    if (home) copy.append(node('time','elan-date',new Intl.DateTimeFormat('fr',{day:'numeric',month:'long',year:'numeric'}).format(new Date())));
    hero.append(copy,art(home ? 'accueil' : scene[0],'elan-hero-art'));
  ...
  function calendrier(hote) {
    ...
    var barre = node('div','elan-month'), titre = node('strong');
    ...
    var table = node('table','elan-calendar'); table.setAttribute('aria-label','Calendrier du mois');
    ...
    function rendre() {
      ...
      for(var r=0;r<6;r++){
        ...
        for(var c=0;c<7;c++){
          ...
          if(debut.getMonth()!==affichage.getMonth())td.className='elan-other';
          if(debut.toDateString()===today.toDateString()){td.className='elan-today';td.setAttribute('aria-current','date');}
  ...
  function aide(hote) {
    var section = node('section','elan-help'), b = node('button','','Aide & tutoriels'); b.type='button';
    section.append(node('h2','','Un coup de main ?'),art('aide','elan-help-art'),node('p','','Des guides pour vous accompagner, à chaque étape.'),b);
  ...
  function accueil(contenu) {
    contenu.classList.add('elan-home');
    if(contenu.querySelector(':scope > .elan-aside'))return;
    var aside=node('aside','elan-only elan-aside'); aside.setAttribute('aria-label','Agenda et accompagnement');
    var agenda=node('section','elan-agenda');agenda.append(node('h2','','Mon agenda'));
    calendrier(agenda);
    var source=document.getElementById('liste-evenements');
    if(source) {
      var events=node('div','elan-events');events.append(node('h3','','Prochains événements'));
      ...
      var original=source.closest('.panneau');if(original)original.classList.add('elan-events-original');
    } else agenda.append(node('p','elan-calendar-caption','Votre repère pour organiser la journée.'));
  ...
  function decorerLiens() {
    ...
    document.querySelectorAll('.tuile[data-page],.tiroir-ligne .lien[href]').forEach(function(lien){
      ...
      mettreArt(lien,p==='#'?'aide':scenePour(p)[0],'elan-tile-art');
    });
    document.querySelectorAll('.carte-stat').forEach(function(c,i){mettreArt(c,statScenes[i%statScenes.length],'elan-stat-art');});
  }
  function decorerAdmin(){
    var hero=document.querySelector('.sa-principal > .elan-hero');
    if(!hero)return;
  ...
  function installer() {
    ...
    if(nav && !nav.querySelector('.elan-profile-link')){
      var profil=node('a','elan-only elan-profile-link','Mon profil');profil.href='mon-profil.html';nav.append(profil);
    }
    decorerLiens();
    if(!navObserver) {
    ...
    if(page==='mon-profil' && !document.querySelector('.elan-layout-note')) {
      var grille=document.getElementById('grille-themes');
      if(grille)grille.after(node('p','elan-only elan-layout-note','Élan utilise une navigation en haut sur ordinateur. Vos préférences de barre latérale sont conservées pour les autres apparences.'));
    }
    if(page==='super-admin'){
      decorerAdmin();
      if(!adminObserver){
  ...
  window.ArdoiseElan={scenes:Object.keys(scenes),installer:installer};
})();
