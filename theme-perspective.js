...
(function () {
  ...
  var base = new URL('public/perspective/', document.currentScript.src);
  var page = location.pathname.split('/').pop().replace(/\.html$/, '');
  var scenes = {
    'dashboard-directeur':['accueil','Une vision claire de votre école.','L’essentiel pour piloter votre journée.'],
  ...
  function calendrier(hote) {
    ...
    var barre = node('div','perspective-month'), titre = node('strong');
    ...
    var table = node('table','perspective-calendar'); table.setAttribute('aria-label','Calendrier du mois');
    ...
    function rendre() {
      ...
      for(var r=0;r<6;r++){
        ...
        for(var c=0;c<7;c++){
          ...
          if(debut.getMonth()!==affichage.getMonth())td.className='perspective-other';
          if(debut.toDateString()===today.toDateString()){td.className='perspective-today';td.setAttribute('aria-current','date');}
  ...
  function aide(hote) {
    var section = node('section','perspective-help'), b = node('button','','Aide & tutoriels'); b.type='button';
    section.append(node('h2','','Un accompagnement à chaque étape.'),art('aide','perspective-help-art'),node('p','','Des guides pour vous accompagner, à chaque étape.'),b);
  ...
  function accueil(contenu) {
    contenu.classList.add('perspective-home');
    if(contenu.querySelector(':scope > .perspective-aside'))return;
    var aside=node('aside','perspective-only perspective-aside'); aside.setAttribute('aria-label','Agenda et accompagnement');
    var agenda=node('section','perspective-agenda');agenda.append(node('h2','','Mon agenda'));
    calendrier(agenda);
    var source=document.getElementById('liste-evenements');
    if(source) {
      var events=node('div','perspective-events');events.append(node('h3','','Prochains événements'));
      ...
      var original=source.closest('.panneau');if(original)original.classList.add('perspective-events-original');
    } else agenda.append(node('p','perspective-calendar-caption','Votre repère pour organiser la journée.'));
  ...
  function decorerLiens() {
    ...
    document.querySelectorAll('.tuile[data-page],.tiroir-ligne .lien[href]').forEach(function(lien){
      ...
      mettreArt(lien,p==='#'?'aide':scenePour(p)[0],'perspective-tile-art');
    });
    document.querySelectorAll('.carte-stat').forEach(function(c,i){mettreArt(c,statScenes[i%statScenes.length],'perspective-stat-art');});
  }
  function decorerAdmin(){
    var hero=document.querySelector('.sa-principal > .perspective-hero');
    if(!hero)return;
  ...
  function installer() {
    ...
    if(nav && !nav.querySelector('.perspective-profile-link')){
      var profil=node('a','perspective-only perspective-profile-link','Mon profil');profil.href='mon-profil.html';nav.append(profil);
    }
    decorerLiens();
    if(!navObserver) {
  ...
  window.ArdoisePerspective={scenes:Object.keys(scenes),installer:installer};
})();
